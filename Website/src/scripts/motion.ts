/**
 * Site motion — GSAP + ScrollTrigger + Lenis.
 * Rules (see Website/DESIGN.md): ease out, transform+opacity only,
 * reveals fire once, everything gated on prefers-reduced-motion.
 *
 * Lifecycle: the site uses Astro's ClientRouter (view transitions), so this
 * module is evaluated ONCE and pages are swapped under it. Everything
 * page-scoped lives inside initPage(), re-run on astro:page-load; the
 * matchMedia context is reverted (killing its tweens and ScrollTriggers) on
 * astro:before-swap. Lenis and the gsap ticker are cross-page singletons.
 */
...
// Smooth scroll — interpolated wheel/touch scrolling so the pinned scrubs
// read as continuous motion instead of wheel-notch steps. Reduced-motion
// users keep native scrolling.
...
let unmountMesh: (() => void) | undefined;

function initPage() {
  // ClientRouter diffs <html> attributes on swap, wiping the classes Lenis
  // put there at init — restore them so the lenis CSS keeps applying.
  ...
  mm.add('(prefers-reduced-motion: no-preference)', () => {
    ...
    // fallback that shows through the untouched canvas. The cleanup fn stops
...
// ClientRouter lifecycle: astro:page-load fires on initial load and after
// every client-side navigation; astro:before-swap right before the old DOM
// is replaced.
document.addEventListener('astro:page-load', initPage);
document.addEventListener('astro:before-swap', () => {
  mm?.revert(); // kills this page's tweens + ScrollTriggers (and pin spacers)
  mm = null;
  unmountMesh?.();
  unmountMesh = undefined;
});
